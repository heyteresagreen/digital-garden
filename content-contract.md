...
- **`![[image.jpg]]`** or **`![[image.jpg|alt text]]`** — image embed. **Known limitation:** Obsidian's own width-hint syntax, `![[image.jpg|alt text|300]]`, is *not* specially handled — the `|300` gets absorbed into the alt text literally (alt becomes `"alt text|300"`), not interpreted as a width. If you use that syntax in Obsidian for local sizing, check the published alt text afterwards.

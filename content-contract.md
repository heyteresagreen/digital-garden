...
- **`imageUrl`** — takes priority if set. Must be a site-rooted path (`/img/user/assets/...`). **No longer needed for new notes** — see below.
- **`image`** — the only field a new note needs. Accepts: an already-resolved path (`/img/user/...`); Obsidian's own embed syntax (`"[[ux-camp-23 - 4.jpeg|alt text]]"`, with or without a size hint like `|300`, which is parsed but not applied to thumbnails — thumbnails are sized by CSS per list style instead); or a bare vault-relative path with no brackets (`assets/foo.jpg`), for the handful of older notes written that way.
- **`imageAlt`** — if set, overrides whatever alt text `image`/`imageUrl` would otherwise produce.

**Why use `image` alone, not `imageUrl`:** `image` written as a real `[[wiki link]]` is something Obsidian actually tracks — it shows up in backlinks/graph view for that image, and Obsidian's automatic link-updating keeps it correct if you rename or move the file. A plain path string in `imageUrl` is invisible to Obsidian; rename the file and it goes stale silently. (July 2026: every one of the 70 notes that had both fields was cleaned up to keep only `image` — see the `feature/dedupe-image-frontmatter` commit. Three were left untouched because `image` and `imageUrl` actually disagreed — worth knowing two of those, "Fountain pens and ink" and "Starting with Sketchnotes", have the same image filename duplicated in two folders, so the resolver can't tell which one you meant; and "Perth Web Accessibility Camp 2023"'s `image` field points at a filename that's since been renamed on disk. None of these are urgent — `imageUrl` still works correctly in all three — but they're worth a look next time you're in those notes. Separately, "Tiny watercolour kit"'s thumbnail was already broken before any of this — `imageUrl`/`image` both point at a file that doesn't exist; the likely real file is `assets/watercolour 2-06e4a6cd.jpeg`, but that's a guess, not confirmed.)
